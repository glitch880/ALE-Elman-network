...
//functions to retrieve pixel values. Nothing interesting 
		...
		for (int episode = 0; episode < ga.getPopulationSize(); episode++)
		{
			...
			{			
				...
				for (size_t i = 0; i < temp.height(); i++)
				{
					...
					//	pixelVectorT.push_back(PixelRep(colorVec[(160 * 3)*i + j * 3], colorVec[(160 * 3)*i + j * 3 + 1], colorVec[(160 * 3)*i + j * 3 + 2]));
				...
				assert(pixelVector.size() == temp.height() * temp.width());

				std::vector<PixelRep> downsampledInput(DownSampler::getDownsampledGrayScale(pixelVector, DOWNSAMPLING_FACTOR, customGrayscale));  //downsample the screen info
